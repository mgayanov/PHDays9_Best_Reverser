...
def sign_key(input_data_xored):
	...
	second_correction = [0x04, 0x04, 0x04, 0x1C, 0x1A, 0x1A, 0x06, 0x42, 0x02]
	...
	for i in range(9):
		...
		for _ in range(c[0]):
			...
			d1 = input_data_xored & 0xff

			d2 = d0 ^ d1

			main_cycle_xored = sub_E3E(prev, d2)

			prev = main_cycle_xored

			d2_xored_storage.append(d2)

		corr = second_correction.pop(0)

		for _ in range(c[1]):

			offset += 1

			d2 = d2_xored_storage[-corr]
		...
		input_data_xored = transform_input_xored(input_data_xored)
		
		return main_cycle_xored
