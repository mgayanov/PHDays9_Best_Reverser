...
Это легко доказать, если поставить брейк на запись по адресу `0x00FFFF79`: брейк сработает на
инструкции `0x00001F94 move.b 1(a2), 0x2F(sp)`.
В регистре `a2` в этот момент хранится адрес `0x00FF0D46` - адрес первого хэша,
...
Этот блок меняет содержимое по адресу 0x00FF0D46(регистр a2), а именно там хранится первый хэш ключа.
...
То есть если в `d0` хранится 4 - берем четвертый с конца элемент.
И стоит помнить, что массив меняется на каждой итерации.

Раз так, то какие конкретно принимает значения `d0`?
Здесь я обошелся без скриптов, просто выписал их.
